...
class DBBookTicker:
    ...
    def create_spread_timeseries_chart(self, access_token: str, dataset_id: int):
        """
        Creates a time-series line chart showing the average spread
        (best_ask_price - best_bid_price) over time, grouped by symbol.
        Based on the configuration observed from chart ID 5.
        """
        self.logger.info(
            f"Attempting to create spread time-series chart for dataset {dataset_id}"
        )
        ...
        payload = {
            "slice_name": f"{self.title.capitalize()} Average Spread Over Time",  # Chart name
