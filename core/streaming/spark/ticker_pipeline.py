...
class TickerPipeline(PipelineBase):
    ...
    def to_line_protocol(self, row: Row):
        ...
        # Dùng close_time làm timestamp chuẩn
    ...
    def run_streams(self):
        ...
        for symbol in TopicCreator.TOPCOIN:
            ...
            # raw_df = raw_data["df"]
            # self.logger.info(f"✅ Raw data schema for {symbol}:")
            # raw_df.printSchema() # Logging schema can be verbose, enable if needed
            ...
            query_s3 = df_to_s3.writeStream.foreachBatch(
                lambda df, epoch_id: self.load_to_S3(df)
            ).start()
