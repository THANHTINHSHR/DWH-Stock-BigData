    ...
    def create_buckets(self):
        ...
        for stream_type in self.STREAM_TYPES:
            if stream_type not in existing_buckets:
                ...
                print(f"✅ Created bucket: {stream_type}")
            else:
                print(f"⚠️ Bucket already exists: {stream_type}")

    def send_line_data(self, bucket_name, line_data):
        pass
        try:
            self.write_api.write(bucket=bucket_name, record=line_data)
            print(f"⚠️ Line Protocol: {line_data}")
            print(f"✅ Line data sent to bucket: {bucket_name}")
        except Exception as e:
            print(f"❌ Failed to send data to bucket {bucket_name}: {e}")
